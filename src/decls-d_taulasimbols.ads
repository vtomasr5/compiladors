...
package decls.d_taulasimbols is
   ...
   procedure posa (ts: in out taula_simbols; id: in id_nom; d: in descripcio;
                   error: out boolean);
   ...
   function consulta_camp (ts: in taula_simbols; idr, idc: in id_nom)
                           return descripcio;
   procedure actualizar(ts: in out taula_simbols; id: in id_nom; d: in descripcio);

   procedure posa_index (ts: in out taula_simbols; ida: in id_nom;
                         di: in descripcio);
   procedure primer_index (ts: in taula_simbols; ida: in id_nom;
                           it: out index_expansio);
   ...
   procedure consulta_index (ts: in taula_simbols; it: in index_expansio;
                             di: out descripcio);

   procedure posar_paramf (ts: in out taula_simbols; idproc, idparf: in id_nom;
                           dparf: in descripcio; error: out boolean);
   procedure primer_paramf (ts: in taula_simbols; idproc: in id_nom;
                            it: out index_expansio);
   procedure seguent_paramf (ts: in taula_simbols; it: in out index_expansio);
   procedure consulta_paramf (ts: in taula_simbols; it: in index_expansio;
                              idparf: out id_nom; dparf: out descripcio);
